  ...
  return (
    <>
      {loading ? (
        ...
        <div className="w-full max-w-4xl mx-auto mt-10 bg-gradient-to-r from-green-300 to-blue-500 p-4 rounded">
          <input
            value={search}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {data
              .filter((item) => {
                ...
                return item.protype === "Mobile" && itemName.includes(searchQuery);
              })
              .map((item) => (
                <motion.div
                  key={item.id}
